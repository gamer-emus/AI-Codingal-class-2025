import pandas as pd
from sklearn.feature_extraction.text import TfidfVectorizer #Looks at the word and calculates the number of times it has been used and calculates the least ones. 
...
def recommend_movie(genre = None, Mood = None, rating = None , top_n = 5):
    ...
    filtered_df = filtered_df.sample(frac=1).reset_index(drop=True) #randomised the orders
    


    recommendations = []
    for idx , row in filtered_df.itarrows():
    ...
    return recommendations if recommendations else "No sutiable Recomandations found"


def display_recommendations(recs, name):
    print(Fore.YELLOW + f"\n AI Analyzied Movie Recommendations for {name}:")
    for idx, (title, polarity) in enumerate(recs,1):
        sentiment = "Positive " if polarity > 0 else "negetive " if polarity < 0 else  "Neutral"
...
def handle_ai(name):
    ...
    print(Fore.GREEN + "avalible Genres : ", end = "")
    for idx , genres in enumerate(genres , 1):
    ...
    while True:
        ...
        if genre_input.isdigit() and 1 <= int(genre_input) <= len(genres):
            ...
            break
        print(Fore.RED + "Invalid input pleas try again ")
...
moon_desc = "Positive :D" if polarity > 0 else "Negetive" if polarity < 0 else "Neutral :|"
print(f"/n {Fore.GREEN} Your mood is {moon_desc} polarity = {polarity:.2f} \n")
...
proccesing_anim()
recs = recommend_movie(genre=genres , mood=mood , rating=rating , top_n=5)
...
while True:
    action = input(Fore.YELLOW + "\n Would You like movie recomendations? (yes/no): ").stip().lower()
    if action == 'no':
...
name = input(Fore.YELLOW + "Whats ur Name?").strip()
