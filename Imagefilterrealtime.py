...
hands = mp_hands(min_detection_confidence = 0.7 , min_tracking_confidence = 0.7)
...
if not cap.isOpened():
    print("Error could not accses webcam");exit()
...
def apply_filter(frame , ftype):
    ...
    """Apply The speciied filter to the frame"""
    if ftype == 'GRAYSCALE':
...
while True:
    ...
    if not success:
        print("FAILED TO READ FRAME -_-");break
    img = cv2.flip(img , 1)
    h , w = img.shape[:2]
